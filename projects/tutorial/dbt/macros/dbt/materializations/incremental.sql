...
{% materialization incremental, adapter='clickhouse' %}
  ...
  {% if existing_relation is none %}
    ...
    {% if config.get('insert_by_period__time_column') %}
...
{% macro clickhouse__get_period_boundaries(time_column, start_date, stop_date, period) -%}
  {% call statement('period_boundaries', fetch_result=True) -%}
    {# with data as (
      select
        date_trunc('second', min('{{ time_column }}')) as start_timestamp,
        date_trunc('second', max('{{ time_column }}')) + interval '1 second' as stop_timestamp
      from {{ source }}
    ) #}
...
{% macro clickhouse__get_period_sql(sql, time_column, period, start_timestamp, stop_timestamp, offset) -%}
  {%- set period_filter -%}
    (
      "{{ time_column }}" >  '{{ start_timestamp }}'::timestamp + interval '{{ offset }} {{ period }}' and
      "{{ time_column }}" <= '{{ start_timestamp }}'::timestamp + interval '{{ offset }} {{ period }}' + interval '1 {{ period }}' and
      "{{ time_column }}" <  '{{ stop_timestamp }}'::timestamp
    )
  {%- endset -%}
...
{% macro clickhouse__insert_by_period_create_table(temporary, relation, sql) %}
  {%- set time_column = config.require('insert_by_period__time_column') -%}
  ...
  {%- set period_boundaries = clickhouse__get_period_boundaries(time_column, start_date, stop_date, period) -%}
  ...
  {% for offset in range(num_periods) -%}
    ...
    {%- set filtered_sql = clickhouse__get_period_sql(sql, time_column, period, start_timestamp, stop_timestamp, offset) -%}
