...
{% macro clickhouse__get_period_sql(sql, column, period, range_min, range_max, offset) -%}
  {%- set batch_predicates -%}
    (
      "{{ column }}" >= toDateTime64('{{ range_min }}', 6) + interval '{{ offset }} {{ period }}' and
      "{{ column }}" <  toDateTime64('{{ range_min }}', 6) + interval '{{ offset + 1 }} {{ period }}' and
      "{{ column }}" <= toDateTime64('{{ range_max }}', 6)
    )
  {%- endset -%}
